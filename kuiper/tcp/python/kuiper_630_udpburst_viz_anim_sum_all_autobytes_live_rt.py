...
ax3d.set_title("Constellation (t=0s)")
...
ax3d.set_ylabel("Y")
ax3d.set_zlabel("Z")
P0 = get_positions_at(0)
sc = ax3d.scatter(P0[:, 0], P0[:, 1], P0[:, 2], s=1) if P0.size else ax3d.scatter([], [], [], s=1)
...
vline = ax_top.axvline(0, linestyle="--")
...
bars = ax_bot.bar(np.arange(0, WINDOW), np.zeros(WINDOW), width=0.9)
...
def update(t):
    ...
    y = in_mbps[:t + 1] if t + 1 <= len(in_mbps) else np.pad(in_mbps, (0, t + 1 - len(in_mbps)), constant_values=0.0)
    x = np.arange(0, len(y), dtype=int)
    line_top.set_data(x, y)
    vline.set_xdata([t, t])

    lo = max(0, t - WINDOW + 1)
    xs = np.arange(lo, t + 1, dtype=int)
    ywin = in_mbps[lo:t + 1] if t + 1 <= len(in_mbps) else np.pad(in_mbps[lo:], (0, t + 1 - len(in_mbps)), constant_values=0.0)

    for b in bars:
        b.set_height(0.0)
        b.set_x(0.0)

    for i, (xi, yi) in enumerate(zip(xs, ywin)):
        bars[i].set_x(xi - 0.45)
        bars[i].set_height(float(yi))
