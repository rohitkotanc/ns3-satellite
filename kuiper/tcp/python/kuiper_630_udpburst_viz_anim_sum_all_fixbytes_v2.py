...
def read_pkt_csv(path, colname):
    ...
    df["t_s"] = (df[colname] / 1e9).astype(float)
    df["t_int"] = np.floor(df["t_s"]).astype(int)
    return df[["t_int"]]

def sum_throughput_mbps(pattern, colname):
    ...
    for fp in files:
        ...
        for k, v in g.items():
            per[int(k)] = per.get(int(k), 0) + int(v)
    idx = np.arange(0, t_end + 1, dtype=int)
    pkts = np.array([per.get(int(t), 0) for t in idx], dtype=float)
    mbps = (pkts * UDP_PAYLOAD_BYTES * 8.0) / 1e6
    return mbps, idx

frame_by_t, all_max = load_frames()
in_mbps, t_vals = sum_throughput_mbps("udp_burst_*_incoming.csv", "recv_ns")
out_mbps, t_vals2 = sum_throughput_mbps("udp_burst_*_outgoing.csv", "sent_ns")

T_END = int(max(t_vals.max(), t_vals2.max(), max(frame_by_t.keys()) if frame_by_t else 0))
...
def draw_at_time(t):
    ...
    ys = in_mbps[lo:t + 1] if (t + 1) <= len(in_mbps) else np.pad(in_mbps[lo:], (0, (t + 1) - len(in_mbps)), constant_values=0.0)
