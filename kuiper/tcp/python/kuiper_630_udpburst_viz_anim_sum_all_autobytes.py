...
def series_at_time(series, t):
    if t < 0:
        return np.array([], dtype=float)
    if t >= len(series):
        return np.pad(series, (0, t + 1 - len(series)), constant_values=0.0)
    return series
...
def draw_at_time(t):
    ...
    ax_top.cla()
    y_in = series_at_time(in_mbps, t)
    x_in = np.arange(0, len(y_in), dtype=int)
    ax_top.plot(x_in, y_in)
