...
def load_rtt_series(path: str):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            # Handle possible ns / us style scales defensively
            if t > 1e6:
                t /= 1e9
            if rtt > 1e6:
                rtt /= 1e6
...
def parse_ground_stations(path: str):
    grounds = []
...
def sat_position_m(satrec_obj, when_utc: dt.datetime):
    jd, fr = jday(
        when_utc.year,
        when_utc.month,
        when_utc.day,
...
def extract_start_time(description_path: str):
...
def main():
    ...
    if not sat_positions_series or not sat_positions_series[0]:
        raise ValueError("No satellite positions generated from TLEs")
    ...
    # Only highlight src/dst if they line up with full node-id style indexing
    src_sc = None
    dst_sc = None
    sat_count = len(tles)
    ...
    finite = rtt_ms[np.isfinite(rtt_ms)]
    if finite.size:
        ymin = float(np.min(finite))
        ymax = float(np.max(finite))
        if abs(ymax - ymin) < 1e-9:
            ax2d.set_ylim(ymin - 5.0, ymax + 5.0)
        else:
            pad = 0.1 * (ymax - ymin)
