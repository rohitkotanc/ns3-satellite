...
def visible_from_ground(gs_pos, sat_pos, min_elevation_deg=25.0):
    return elevation_deg(gs_pos, sat_pos) >= min_elevation_deg
...
def build_graph(nodes, n_sats=1584, k_neighbors=4, min_elevation_deg=25.0, gs_links=2,
                isl_rate_gbps=10.0, gsl_rate_gbps=10.0):
    G = nx.Graph()
    ...
    for i, nid in enumerate(sat_ids):
        ...
        for j in order:
            ...
            delay = d / C
            G.add_edge(nid, nid2, weight=delay, capacity_gbps=isl_rate_gbps, link_type="isl")
            nbr_count += 1
    ...
    for gs in gs_ids:
        ...
        for d, sat, elev, cap in visible[:gs_links]:
            delay = d / C
            G.add_edge(gs, sat, weight=delay, capacity_gbps=cap, link_type="gsl", elevation_deg=elev)
...
def compute_udp_throughput_gbps(G, src, dst, offered_rate_gbps=20.0, alpha=0.12):
    try:
    ...
    if not caps:
        return np.nan, path, np.nan, np.nan

    bottleneck = min(caps)
    hops = len(path) - 1
    effective_capacity = bottleneck / (1.0 + alpha * max(0, hops - 1))
    throughput = min(offered_rate_gbps, effective_capacity)

    return throughput, path, bottleneck, hops
...
def main():
    ...
    ap.add_argument("--offered_rate_gbps", type=float, default=20.0)
    ...
    for j, idx in enumerate(frame_idxs):
        ...
        thr, path, bottleneck, hops = compute_udp_throughput_gbps(
            G,
            src,
            dst,
            offered_rate_gbps=args.offered_rate_gbps,
            alpha=args.alpha
        )
        ...
        if j % 10 == 0 or j == len(frame_idxs) - 1:
            if np.isfinite(thr):
                print(f"Computed frame {j+1}/{len(frame_idxs)}   throughput={thr:.3f} Gbps   hops={hops}")
            else:
                print(f"Computed frame {j+1}/{len(frame_idxs)}   throughput=no path")
    ...
    ymax = max(args.offered_rate_gbps, args.isl_rate_gbps, args.gsl_rate_gbps)
    if finite.size:
    ...
    def update(i):
        ...
        if p is not None and len(p) >= 2:
            px = [pos[n][0] for n in p if n in pos]
            py = [pos[n][1] for n in p if n in pos]
            pz = [pos[n][2] for n in p if n in pos]
            path_line, = ax3d.plot(px, py, pz, linewidth=2)
